...
void* iot_storage(void *arg, std::vector<std::string> senderIP, std::vector<int> senderPort, 
    ...
    std::cout << "Storage: Received the sender socket " << std::endl;

    zmq::socket_t receiver = key_receiver_conn(param, context, receiverIP.front(), receiverPort.front());

    std::cout << "Starting the storage worker " << std::endl;

    std::cout << "Reading messages, storage" << std::endl;
    ...
    while (1) {
        ...
        storage_enclave_execute(smessage, n, &sender, &retmessage, &retlen_a, &retlen);
