...
void* iot_emergency(void *arg, std::vector<std::string> senderIP, std::vector<int> senderPort, 
    ...
    std::cout << "Emergency: Received the sender socket " << std::endl;

    zmq::socket_t receiver = key_receiver_conn(param, context, receiverIP.front(), receiverPort.front());

    std::cout << "Starting the emergency worker " << std::endl;

    std::cout << "Reading messages, emergency" << std::endl;
    ...
    while (1) {
        ...
        emergency_enclave_execute(smessage, n, &sender, &retmessage, &retlen_a, &retlen);
